    ...
    try:
        with open(file_path, "rb") as audio_file:
            response = client.audio.transcriptions.create(
                ...
                # temperature=0 curbs whisper-large-v3 looping. NO `prompt`: on
                # silent/low-speech audio Whisper regurgitates its prompt as the
                # transcript, which leaked instruction text into output.
                temperature=0,
            )
    except Exception as exc:
        record_api_call(
